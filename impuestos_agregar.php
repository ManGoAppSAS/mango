...
if ($agregar == 'si')
{
    $consulta = $conexion->query("SELECT * FROM impuesto WHERE impuesto = '$impuesto' and porcentaje = '$porcentaje'");

    if ($consulta->num_rows == 0)
    {
        $insercion = $conexion->query("INSERT INTO impuesto values ('', '$ahora', '', '', '$sesion_id', '', '', 'activo', '$impuesto', '$porcentaje')");        

        $mensaje = "Impuesto <b>" . ($impuesto) . "</b> agregado";
        $body_snack = 'onLoad="Snackbar()"';
        $mensaje_tema = "aviso";
    }
    else
    {
        $mensaje = "El impuesto <b>" . ($impuesto) . "</b> ya existe, no es posible agregarlo de nuevo";
        $body_snack = 'onLoad="Snackbar()"';
        $mensaje_tema = "error";
    }
}
